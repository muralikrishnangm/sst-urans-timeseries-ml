# # Introduction
"""
* Testing 1D ML-URANS model with random inputs and comparing with true data
* **NOTE**: For modeling energies
Sample run script:
(base)[user]$ python nnTesting_apriori.py --casenum 1 --casenum_test 1 --target_T 1.0 --seq_len_T 16 --normEnergy --interpIO --set_dt_seq --dt_T 0.5 --nepoch 5 --batch_size 10 --eval_regime training --saveData --HDdir . --disable_cuda
"""
...
fnameOP      = f'Out_Phydt{modelfname_full[17:]}_off_regime{eval_regime}_testdata'
for i in range(len(casenum_test)):    # casename(s)
